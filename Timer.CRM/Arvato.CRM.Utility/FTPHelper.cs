...
namespace Arvato.CRM.Utility
{
    ...
    {
        //基本设置
        static private string path = "";    //目标路径
        static private string ftpip = "";    //ftp IP地址
        ...
        //从ftp服务器上下载文件的功能  
        public bool Download(string fileName, string DownfilePath)
        {
            FtpWebRequest reqFTP;
            try
            {
                //string filePath = System.Web.HttpContext.Current.Server.MapPath("/Upload");
                string filePath = System.Environment.CurrentDirectory;
                Log4netHelper.WriteInfoLog("filePath:" + DownfilePath + "||   path:" + path + "||   filename" + fileName);
                if (!Directory.Exists(DownfilePath))//如果不存在就创建file文件夹
                {
                    Directory.CreateDirectory(DownfilePath);
                }

                FileStream outputStream = new FileStream(DownfilePath +"//"+ fileName, FileMode.Create);
