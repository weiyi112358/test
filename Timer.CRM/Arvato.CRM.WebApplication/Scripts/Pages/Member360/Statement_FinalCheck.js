    ...
    $("#frmAddStore").submit(function (e) {
        ...
        if (DataValidatorAdd.form()) {
            ...
            if ($("#txtTradeId").val() == "") {
                ...
                ajax("/Member360/EditCheckedData", { trade: trade, mid: $("#txtTradeId").val() }, function (res) {
                    if (res.IsPass) {
                        $.colorbox.close();
                        LoadTrade();
                        $.dialog(res.MSG);
...
function LoadTrade() {
    if (!dt_StoreData) {
        ...
        dt_StoreData = $('#dt_StoreData').dataTable({
            sAjaxSource: '/Member360/GetStatementData_FinalCheck',
            sScrollX: "100%",
            sScrollXInner: "100%",
            ...
            { data: 'StatusSalesText', title: "审批状态", sortable: true, },
            {
                ...
                render: function (obj) {
                    if (obj.StatusSales == "1")
                        ...
                        return "<button class=\"btn\" onclick=\"vehcile('" + obj.MemberID + "')\">查看车辆</button><button class=\"btn btn-default\" onclick=\"Check(" + obj.TradeID + ")\">查看</button>";
            ...
            fnFixData: function (d) {
                ...
                d.push({ name: 'tradeCode', value: $("#drpTradeCode").val() });
            }
        });
    } else {
        dt_StoreData.fnDraw();
...
function Check(v) {
    ...
    ajax("/Member360/GetStatementEditInfo", { mid: v }, function (data) {
        if (data != null) {
            ...
            $("#txtAmount2").val(item.His_Amount),
            $("#txtPrice2").val(item.His_StandardAmountSales),
            $("#txtNonGold2").val(item.His_NotGoldAmountSales),
            $("#txtGold2").val(item.His_GoldAmountSales),

            $("#drpStore").val(item.StoreCodeSales);
            $("#dropPayType").val(item.PayMentTypeSales);
            $("#dropPayType2").val(item.His_PayMentTypeSales);
            ...
            $("#drpBusinessType").attr("disabled", true);
            $("#drpBusinessChildType").attr("disabled", true);
            $("#drpVehicle").attr("disabled", true);
            $("#drpStore").attr("disabled", true);
            $("#drpBusinessNo").attr("readonly", true);
            $("#txtDate").attr("readonly", true);
            $("#txtAmount").attr("readonly", true);
            $("#txtPrice").attr("readonly", true);
            $("#txtAmount2").attr("readonly", true);
            $("#txtPrice2").attr("readonly", true);
            $("#txtNonGold2").attr("readonly", true);
            $("#txtGold2").attr("readonly", true);
            $("#dropPayType2").attr("disabled", true);
