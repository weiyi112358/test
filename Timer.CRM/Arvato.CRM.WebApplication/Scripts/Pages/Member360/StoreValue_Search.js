...
$(function () {
    ...
    $("#btnSubmit2").click(function () {
        ...
        if (id != '') {
            ...
            //储值操作
            ajax('/Member360/SaveEditValue', { mid: id, changeValue: changeValue, storeCode: $("#drpStore2").val(), date: $("#txtDate2").val() }, function (res) {
                if (res.IsPass) {
                    flag = 1;
                    LoadStoreValueRecord();
                    $.dialog(res.MSG);
...
function LoadStoreValueRecord() {
    if (!dtStoreData) {
        dtStoreData = $('#dt_StoreData').dataTable({
            sAjaxSource: '/Member360/GetStoreValueRecord_Search',
            bSort: true,   //不排序
            bInfo: true,   //不显示 ‘从 _START_ 到 _END_ 条，共 _TOTAL_ 条记录’
            bServerSide: true,  //每次请求后台数据
            bLengthChange: false,//不显示 ‘显示 _MENU_ 条’
                ...
                { data: "StatusSalesText", title: "审批状态", sortable: true },
                {
                    data: null, title: "操作", sortable: false, render: function (obj) {
                        if (obj.StatusRecharge == "1")
                            return "<button class=\"btn btn-default\" onclick=\"Edit(" + obj.TradeID + ")\">查看</button>";
                        else
                            return "";
                    }
                }
...
function Edit(id) {
    ...
    $("#drpStoreLevel2").attr("disabled", "disabled");
    $("#txtDate2").attr("disabled", "disabled");
    $("#drpStore2").attr("disabled", "disabled");
