...
$(function () {
    ...
    dtUsers = $('#dt_users').dataTable({
        ...
        iDisplayLength: 8,
        aoColumns: [
            { data: 'UserID', title: "用户编号", sortable: true},
            { data: 'UserName', title: "用户名", sortable: false },
            { data: 'LoginName', title: "登录名", sortable: false},
            ...
            {
                data: null, title: "操作", sortable: false, sWidth: "35%", render: function (r) {
                    var str = '<button class="btn" onclick="edit(' + r.UserID + ')">编辑</button> ';
                    str += '<button class="btn" onclick="resetPsd(' + r.UserID + ')">重置密码</button> ';
                    str += '<button class="btn" onclick="changeRole(' + r.UserID + ',' + r.DataGroupID + ')">更改角色</button> ';
                    str += '<button class="btn" onclick="changeBusi(' + r.UserID + ',\'' + r.UserName + '\')">分配部门</button> ';
    ...
    //修改业务部门提交
    $("#frmChangeBusi").submit(function (e) {
        e.preventDefault();
        var pageRoles = $("#pageBusis").val();
        if (!pageRoles) {
            pageRoles = [];
            $.dialog('请选择业务部门');
            return;
        }
        ajax("/Auth/SubmitChangeBusi", { userId: $("#txteditUserID").val(), pageRoles: pageRoles}, function (d) {
            if (d.IsPass) {
                $.dialog(d.MSG);
                //clearFormData("frmChangeBusi");
                $.colorbox.close();
            }
            else {
                $.dialog(d.MSG);
...
//更改角色
function changeRole(userId, dataGroupId) {
    clearFormData("frmChangeRole");
    $("#changeRoleUserId").val(userId);
    //加载角色列表
    ajax("/Auth/GetAllRolesByDataGroupId", { dataGroupId: dataGroupId, roleType: "" }, function (data) {
        var pp = '', dp = '';
        for (var i in data) {
...
//更改业务部门
function changeBusi(userId, userName) {
    clearFormData("frmChangeRole");
    ...
    //加载用户角色
    ajax("/Auth/GetBusiByUserId", { userId: userId }, function (d) {
        for (var i in d) {
            $("#pageBusis option[value=" + d[i].DepartmentID + "]").attr("selected", true);
        }
        $(".chzn_select").trigger("liszt:updated");
