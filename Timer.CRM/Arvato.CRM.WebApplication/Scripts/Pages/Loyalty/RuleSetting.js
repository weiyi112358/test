...
$(function () {
    ...
    $("#btnActive").click(function (e) {
        var postUrl = "/Loyalty/ActiveRuleById";
        ajax(postUrl, { "ruleId": $("#txtRuleID").val() }, ActiveRuleCallBack);
    });

    //取消激活
    $("#btnActive1").click(function (e) {
        var postUrl = "/Loyalty/InActiveRuleById";
        ajax(postUrl, { "ruleId": $("#txtRuleID").val() }, ActiveRuleCallBack1);
    });
    ...
    //增量只能实时，非增量的什么都可以
    $("#selRuleType").click(function () {
        var type = $("#selRuleType").val();
        if (type == "1") {
    ...
    $('#btnAddAct').click(function () {
        ModifidAct = "";
        $("#btnAddAction").text("添加");

        $("#divOp,#divRightValue1,#divRightValue2,#divRightValue3,#divRightValue4,#divRightValue5,#divRightValue6,#divRightValue7,#divRightValue8,#divRightValue9,#divRightValue10,#divRightValue11,#divRightValue12,#divRightValue13,#divRightValue14,#divRightValue15,#divRightValue16").html("");
        $("#divAtten").hide();
        $("#selActionLeft").val("");
        $("#tbAction h3").text("新增动作");
        $.colorbox({
            initialHeight: '0',
            initialWidth: '0',
            href: "#tbAction",
    ...
    $("#btnCancelComRule").click(function () {
        $.colorbox.close();
    });
    $("#divAtten").hide();
    //弹出层左值选择事件
    $("#selActionLeft").change(function () {
        if ($("#selActionLeft").val() == "Account") {
            $("#divOp").html("<label>帐户类型</label>" + strActTypeOperatorHtml);
            $("#divRightValue10").html("<label>账户限制</label>" + strActLimitOperatorHtml);
            $("#divRightValue1").show().html("<label>操作符</label>" + numActionOperatorHtml);
            $("#divRightValue2").show().html("<label>右值</label><select id='drpRightValue' name='' onchange='selRightValueChange(this)'  class='span12'><option value='1'>右值（可选值）</option><option value='2'>右值（固定值）</option></select>");

            //$("#divRightValue3").html("<label>右值</label><select class='span11' style='width: 165px; z-index: -1' id='txtRightValue' name='txtRightValue' onkeydown='deleteText(this)' onkeypress='writeSelect(this)'><option value=''></option></select>");
            //$("#divRightValue3").html("<div style='position: relative;'><label>右值</label><select id='txtRightValue' name='txtRightValue' style='width: 165px;' onchange=\"document.getElementById('input').value=this.value\"><option value=''></option></select><input id='input' name='input' style='position: absolute; width: 136px; height: 26px; left: 1px; top: 26px; border-bottom: 0px; border-right: 0px; border-left: 0px; border-top: 0px;'></div>");

            //右值可选  onchange='selRightValue2(this)'
            $("#divRightValue3").html("<label>右值(可选值)</label><select id='txtRightValue2' name='txtRightValue2'   class='span12'><option value=''>请选择</option></select>");
            $("#divRightValue4").show().html("<label>右值过滤字段</label><select id='txtRightValueFilterAlias' name='txtRightValueFilterAlias'  class='span12'><option value=''>请选择</option></select>");
            $("#divRightValue5").show().html("<label>右值累计上限(过滤)</label><input id='txtRightMax' type='text' class='span12' onkeyup=\"value=value.replace(/[^\\d.]/g,'')\" />");
            $("#divRightValue6").html("<label>右值累计下限(过滤)</label><input id='txtRightMin' type='text' class='span12' onkeyup=\"value=value.replace(/[^\\d.]/g,'')\" />");
            //右值固定
            $("#divRightValue7").show().html("<label>右值(固定值)</label><input id='txtRightValue1' type='text' class='span12' onkeydown='selRightValue1(this)' onkeyup=\"value=value.replace(/[^\\d.]/g,'')\" />");
            $("#divRightValue8").html("<label>偏移表达式</label>" + strActionOffSetErrHtml);
            $("#divRightValue9").html("<label>偏移量</label><input id='txtOffsetValue' type='text' class='span12' onkeyup=\"value=value.replace(/[^\\-?\\d.]/g,'')\"/>");
            //$("#divRightValue6").html("<label>排序</label><input id='txtSort' type='text' class='span12' />");
            //选时间
            $("#divRightValue11").html("<label>冻结时间</label><input id='txtFreezeValue' type='text' class='span12' onkeyup=\"value=value.replace(/[^\\d.]/g,'')\" />");
            $("#divRightValue12").html("<label>冻结时间单位</label><select id='drpFreezeUnit' name='drpFreezeUnit' class='span12'><option value=''>请选择</option><option value='day'>天</option><option value='month'>月</option><option value='year'>年</option></select>");
            $("#divRightValue13").html("<label>有效时间</label><input id='txtAvailabeValue' type='text' class='span12' onkeyup=\"value=value.replace(/[^\\d.]/g,'')\" />");
            $("#divRightValue14").html("<label>有效时间单位</label><select id='drpAvailabeUnit' name='drpAvailabeUnit' onchange='selAvailabeUnitChange(this)' class='span12'><option value=''>请选择</option><option value='day'>天</option><option value='month'>月</option><option value='year'>年</option></select>");
            ...
            LoadRightValueList();
            //加载右值过滤列表Todo
            LoadRightValueFilterAlias();
            GetLimitList();
            $("#divAtten").show();

            $("#selOp").prop('disabled', true);
            $("#drpRightValue").val('1').change();
            //$("#drpAvailabeUnit").change();

        }
        else if ($("#selActionLeft").val() == "") {
            //清空弹窗信息
            $("#divOp").html('');
            $("#divRightValue1").html('');
            $("#divRightValue2").html('');
            ...
            $("#divRightValue8").html('');
            $("#divRightValue9").html('');
            $("#divRightValue10").html('');
            $("#divRightValue11").html('');
            $("#divRightValue12").html('');
            $("#divRightValue13").html('');
            $("#divRightValue14").html('');
            $("#divRightValue15").html('');
            $("#divRightValue16").html('');
            $("#divAtten").hide();
            $("#selOp").prop('disabled', false);
        }
        else {
            //清空弹窗信息
            $("#divOp").html('');
...
function saveAction() {
    ...
    var ruleMaster = {
        RuleName: encode($("#txtRuleName").val()),
        RuleID: $("#txtRuleID").val(),
        RuleType: $("#selRuleType").val(),
...
function ruleTypeChanged() {
    if ($("#selRuleType").val() == "Increment") {
        $("#rdRealTime").attr("disabled", false);
    }
    else if ($("#selRuleType").val() == "NoIncrement") {
        $("#rdRealTime").attr("checked", false).attr("disabled", true);
        $("#rdCycle").attr("checked", true);
        cycleChanged($("#rdCycle").get(0));
    }
    else {
        $("#rdRealTime").attr("disabled", false);
    }
...
function getMemberSubdLeftValuesAll() {
    var postUrl = "/Loyalty/GetRuleLeftValuesAll";//"/MemSubdivision/GetMemSubdLeftValuesAll";
    ajax(postUrl, null, getMemberSubdLeftValuesAllCallback);
}
...
function getMemberSubdLeftValuesAllCallback(data) {
    ...
    $.each(vlist, function (i, val) {
        if (vlist[i].ControlType == "select") {
            rValCfg.push({ FieldAlias: vlist[i].FieldAlias, TableName: vlist[i].DictTableName, TableType: vlist[i].DictTableType });
        }
    });
    if (rValCfg.length > 0)
    ...
    leftValueList = vlist;
    //leftValueHtml = "<select name='selLeftValue' id='selLeftValue' data-placeholder='选择左值...' class='chzn_a span14'>";
    //aliasHtml = "<option value=''></option>";
    //vlist.forEach(function (item, index, array) {
    //    aliasHtml += "<option l='" + item.FieldAlias + "' f='" + item.FieldName + "' datatype='"
    //        + item.FieldType + "' controltype='" + item.ControlType + "' enumvalue='" + item.EnumValue + "' reg='"
    //        + item.Reg + "'>" + item.FieldDesc + "</option>";
    //});
    //leftValueHtml += aliasHtml + "</select>";

    leftValueHtml = "<input name=\"txtLeftValueSearch\" class=\"search_query input-medium\" id=\"txtLeftValueSearch\" type=\"text\" size=\"16\" placeholder=\"搜索...\" autocomplete=\"off\"/><ul name='selLeftValue' id='selLeftValue' data-placeholder='选择左值...' class='chzn_a span14'>";
    //leftValueHtml += "<option value=''></option>";
    vlist.forEach(function (item, index, array) {
        var tid = "leftRadio" + item.FieldAlias + index;
        var lid = "leftLi" + item.FieldAlias + index;
        leftValueHtml += "<li id='" + lid + "' l='" + item.FieldAlias + "' f='" + item.FieldName + "' datatype='"
            + item.FieldType + "' controltype='" + item.ControlType + "' enumvalue='" + item.EnumValue + "' reg='"
            + item.Reg + "' dictTableName='" + item.DictTableName + "' dictTableType='" + item.DictTableType + "' tableName='" + item.TableName + "'>"
    ...
    //加载行为
    if ($("#hideActions").val() != "") {
        actions = eval("(" + $("#hideActions").val() + ")");
        for (i = 0; i < actions.length; i++) {
            //var lv = Enumerable.from(leftValueList).first("($.FieldAlias == '" + actions[i].l + "')");
            //加载时，
            //$("#dt_act").find("tbody").append("<tr><td>" + lv.FieldDesc + "</td><td>" + getOperatorName1(actions[i].e) + "</td><td>" + actions[i].rt + "</td><td l='" + actions[i].l + "'><button class='btn' type='button' onclick='modifyAct(this)'>编辑</button> <button onclick='removeAct(this)' class='btn btn-danger'>删除</button></td></tr>");
            //修改
            if (actions[i].LeftValue.ExtName != 'Account') {
                var lv = Enumerable.from(leftValueList).first("($.FieldAlias == '" + actions[i].LeftValue.ExtName + "')");
                var rValue = '';
                if (actions[i].OffsetValue == '') {
                    rValue = getOperatorName1(actions[i].RightValue);
                } else {
                    rValue = getOperatorName1(actions[i].RightValue) + actions[i].OffsetExpression + actions[i].OffsetValue + getOperatorName1(actions[i].OffsetUnit);
                }
                $("#dt_act").find("tbody").append("<tr><td>" + lv.FieldDesc + "</td><td>" + getOperatorName1(actions[i].Expression) + "</td><td>" + rValue + "</td><td l='" + actions[i].LeftValue.ExtName + "'><button class='btn' type='button' onclick='modifyAct(this)'>编辑</button> <button onclick='removeAct(this)' class='btn btn-danger'>删除</button></td></tr>");
            } else {
                var desc = GetAliasDescByValue(actions[i].RightValue);
                if (actions[i].OffsetExpression != '') {
                    desc += (actions[i].OffsetExpression + actions[i].OffsetValue);
                }
                $("#dt_act").find("tbody").append("<tr><td>" + getOperatorName1(actions[i].LeftValue.ExtName) + "</td><td>" + getOperatorName1(actions[i].Expression) + "</td><td>" + desc + "</td><td l='" + actions[i].LeftValue.ExtName + "'><button class='btn' type='button' onclick='modifyAct(this)'>编辑</button> <button onclick='removeAct(this)' class='btn btn-danger'>删除</button></td></tr>");
            }
        }
    };

    //加载规则
    if ($("#hideConditionResult").val() != "") {
        actionsResult = eval("(" + $("#hideConditionResult").val() + ")");
        for (i = 0; i < actionsResult.length; i++) {
            //var lv = Enumerable.from(actionsResult).first("($.FieldAlias == '" + actionsResult[i].FieldAlias + "')");
            var desc = GetAliasDescByValue(actionsResult[i].FieldAlias);
            $("#dt_computer").find("tbody").append("<tr><td>" + actionsResult[i].NameCode + "</td><td>" + desc + "</td><td>" + getOperatorName1(actionsResult[i].GroupFunc) + "</td><td>" + getOperatorName1(actionsResult[i].OffsetExpression) + "</td><td>" + actionsResult[i].OffsetValue + "</td><td>" + actionsResult[i].Maximum + "</td><td>" + actionsResult[i].Minimum + "</td><td l='" + actionsResult[i].FieldAlias + "'><button class='btn' type='button' onclick='modifyActResult(this)'>编辑</button> <button onclick='removeActResult(this)' class='btn btn-danger'>删除</button></td></tr>");
        }
    };
}
    ...
    $.post(postUrl, { type: type }, function (res) {
        if (utility.isNull(res)) {
            ...
            for (var i = 0; i < res.length; i++) {
                ...
                var tid = "radio" + i;
                objHtml += "<li itemid='" + res[i].OptionValue + "'><input id='" + tid + "' type='radio' value='" + res[i].OptionValue + "' title='" + res[i].OptionText + "' name='operateGroup' style='float:left;margin-right:5px' /><label for='" + tid + "'>" + res[i].OptionText + "</label></li>";
            }
            objHtml += "</ul>"
        }
        if (type == "StrOperator")
...
/****************************** tab3 start ***********************************/
//弹出层中左值选择改变事件
function actionLeftChanged() {
    //获取选中值
    var leftValue = Enumerable.from(leftValueList).first("($.FieldAlias == '" + $("#selActionLeft").val() + "')");
    //初始化操作符 
    if (dataTypeList.string.indexOf(leftValue.FieldType) >= 0) {
        $("#divOp").html("<label>操作符</label>" + strActionOperatorHtml);
        //初始化右值可选项   
        var options = filterRightVals[leftValue.FieldAlias];
        if (options) {
            var sel = "<label>右值</label><select id='selRightValue1'>";
            for (var i = 0; i < options.length; i++) {
                sel += "<option value='" + options[i].sv + "'>" + options[i].st + "</option>";
            };
            sel += "</select>";
            $("#divRightValue1").html(sel);
        }
        else {
            $("#divRightValue1").html("<label>右值</label><input id='txtRightValue' class='span12' />");
        }
        $("#divRightValue2").html("");
    }
    else if (dataTypeList.date.indexOf(leftValue.FieldType) >= 0) {
        $("#divOp").html("<label>操作符</label>" + strActionOperatorHtml);
        var sel = "<label>右值</label><select id='selRightValue1' class='span12' onchange='selChange(this)'><option value='DateNow'>当前日期</option><option value='NotNow'>指定日期</option></select>";
        $("#divRightValue1").html(sel);
        //$("#divRightValue2").html("");
        //增加可选日期输入框
        $("#divRightValue2").html("<label>可选日期</label><input type='text' placeholder='选择日期' class='span12' id='txtRightValue' />").hide();
        $("#txtRightValue").datepicker();
        //偏移表达式
        $("#divRightValue5").html("<label>偏移表达式</label><select id='selOffset' class='span12'><option value='+'>加</option></select>").hide();

        //偏移量
        $("#divRightValue3").html("<label>偏移量</label><input id='txtOffsetValue' type='text' class='span12' onkeyup=\"value=value.replace(/[^\\-?\\d.]/g,'')\"/>");
        //单位
        $("#divRightValue4").html("<label>单位</label><select id='selDateUnit' class='span12'><option value='year'>年</option><option value='month'>月</option><option value='day'>日</option></select>");
    }

    else {
        $("#divOp").html("<label>操作符</label>" + numActionOperatorHtml);
        //初始化右值可选项    
        var options = Enumerable.from(leftValueList).where("($.IsActionRight == true)").toArray();
        var sel = "<label>右值</label><select id='selRightValue1' class='span12'><option value='fixed'>固定值</option>";
        for (var i = 0; i < options.length; i++) {
            sel += "<option value='" + options[i].FieldAlias + "'>" + options[i].FieldDesc + "</option>";
        };
        sel += "</select>";
        $("#divRightValue1").html(sel);
        $("#divRightValue2").html("<label>固定值/倍数</label><input type='text' id='txtRightValue' class='span12' />").show();
    }
};
...
function selAvailabeUnitChange(obj) {
    var val = $(obj).val();
    if (val == 'month') {
        $("#divRightValue15").html("<label>偏移天数</label><select id='drpAvailUnit2' name='drpAvailUnit2' class='span12'></select>");
        $("#divRightValue16").html('');
        LoadAvailUnit2();
    }
    else if (val == 'year') {
        $("#divRightValue15").html("<label>偏移月数</label><select id='drpAvailUnit1' name='drpAvailUnit1' class='span12'></select>");//offsetday,offsetmonth
        $("#divRightValue16").html("<label>偏移天数</label><select id='drpAvailUnit2' name='drpAvailUnit2' class='span12'></select>");
        LoadAvailUnit1();
        LoadAvailUnit2();
    }
    else {
...
function LoadAvailUnit1() {
    var opt = '';
    for (i = 1; i <= 12; i++) {
        opt += '<option value=' + i + '>' + i + '</option>'
    }
    $('#drpAvailUnit1').append(opt);
}
function LoadAvailUnit2() {
    var opt = '';
    for (i = 1; i <= 31; i++) {
        opt += '<option value=' + i + '>' + i + '</option>'
    }
    $('#drpAvailUnit2').append(opt);
}
//弹出层添加事件
function addAct() {
    if ($("#selActionLeft").val() != "") {
        ...
        //    }
        //}
        //else if ($("#txtRightValue").val() == undefined) {
        //    curRightText = $("#selRightValue1").find("option:selected").text();
        //    curRightValue = $("#selRightValue1").val();
        //}
        //else {
        //    curRightText = $("#selRightValue1").find("option:selected").text() + " × " + $("#txtRightValue").val();
        //    if ($("#txtRightValue").val() == "") {
        //        $.dialog("请填写倍数");
        //        return;
        //    }
        //    curRightValue = $("#selRightValue1").val() + " * " + $("#txtRightValue").val();
        //}
...
$('#btnAddComputer').click(function () {
    ...
    $("#drpGroupFunc").val('');
    $("#drpOffsetExpression").val('');
    $("#txtShift").val('');
    $("#txtCeiling").val('');
    $("#txtFloor").val('');
    $("#txtNameCode").val('');

    $("#btnAddComRule").text("添加");
    $("#table_addcomputer h3").html("设定规则新增");
    $.colorbox({
        initialHeight: '0',
        initialWidth: '0',
...
//弹出层添加交易订单计算
function addActResult() {
    if ($("#txtKey").val() != "") {
        var FieldAlias = $("#txtKey").val();
        if (ModifiActResult != $("#txtKey").val()) {
            if (Enumerable.from(actionsResult).where("($.FieldAlias == '" + FieldAlias + "')").count() != 0) {
                $.dialog("已经添加过此条目");
                return;
            }
        }
        var FieldAlias = $("#txtKey").val();
        var FieldDesc = $("#txtKey").find("option:selected").text();
        var Maximum = $("#txtCeiling").val();
        var Minimum = $("#txtFloor").val();
        var GroupFunc = $("#drpGroupFunc").val();//$("#drpGroupFunc").find("option:selected").text();;
        var GroupFuncText = $("#drpGroupFunc").val() == '' ? '' : $("#drpGroupFunc").find("option:selected").text();
        var OffsetExpression = $("#drpOffsetExpression").val();//$("#drpOffsetExpression").find("option:selected").text();;
        var OffsetExpressionText = $("#drpOffsetExpression").val() == '' ? '' : $("#drpOffsetExpression").find("option:selected").text();
        var OffsetValue = $("#txtShift").val();
        var IsABS = $("#chbIsABS").prop('checked');
        //var NameCode = $("#txtKey").val() + '_' + utility.newGuid;
        NameCode = $("#txtNameCode").val();
        ...
        if (OffsetValue != '') {
            if (!utility.isNumber(OffsetValue)) {
                $.dialog("偏移量格式不正确");
                return;
            }
        }
        //添加或保存
        if (ModifiActResult == "") {
            $("#dt_computer").find("tbody").append("<tr><td>" + NameCode + "</td><td>" + FieldDesc + "</td><td>" + GroupFuncText + "</td><td>" + OffsetExpressionText + "</td><td>" + OffsetValue + "</td><td>" + Maximum + "</td><td>" + Minimum + "</td><td l='" + FieldAlias + "'><button class='btn' type='button' onclick='modifyActResult(this)'>编辑</button> <button onclick='removeActResult(this)' class='btn btn-danger'>删除</button></td></tr>");
        }
        else {
            actionsResult = Enumerable.from(actionsResult).where("($.FieldAlias != '" + ModifiActResult + "')").toArray();
            var line = $("#dt_computer").find("td[l='" + ModifiActResult + "']").parent();
            line.html("<td>" + NameCode + "</td><td>" + FieldDesc + "</td><td>" + GroupFuncText + "</td><td>" + OffsetExpressionText + "</td><td>" + OffsetValue + "</td><td>" + Maximum + "</td><td>" + Minimum + "</td><td l='" + FieldAlias + "'><button class='btn' type='button' onclick='modifyActResult(this)'>编辑</button> <button onclick='removeActResult(this)' class='btn btn-danger'>删除</button></td>");
        }
        actionsResult.push({ FieldAlias: FieldAlias, GroupFunc: GroupFunc, OffsetExpression: OffsetExpression, OffsetValue: OffsetValue, Maximum: Maximum, Minimum: Minimum, IsABS: IsABS, NameCode: NameCode });

    } else {
        $.dialog("请尽量维护完整信息,字段别名必选");
