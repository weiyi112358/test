...
$(function () {
    ...
    dt_attribute = $('#dt_attribute').dataTable({
        sAjaxSource: '/BaseData/GetCampaignAttributeList',
        bSort: true,   //不排序
...
function GetRelationship() {
    ...
    ajaxSync("/BaseData/GetCampaginOptionDataList", { optType: "DynamicTable" }, function (res) {
        if (res.length > 0) {
            var opt = "";//"<option value='-1'>请选择</option>";
            for (var i = 0; i < res.length; i++) {
                opt += '<option value=' + res[i].AliasSubkeyBase + ',' + res[i].AliaskeyBase + '>' + res[i].CampAttrName + '</option>';
            }
...
function GetOptionData(type) {
    $('#drp_catgkey').empty();
    ajaxSync("/BaseData/GetCampaginOptionDataList", { optType: type }, function (res) {
        if (res.length > 0) {
            var opt = "";//"<option value='-1'>请选择</option>";
            for (var i = 0; i < res.length; i++) {
                opt += '<option value=' + res[i].AliasSubkeyBase + ',' + res[i].AliaskeyBase + '>' + res[i].CampAttrName + '</option>';
...
function GetOptionSubData(type) {
    ...
    ajaxSync("/BaseData/GetCampaginOptionDataList", { optType: type }, function (res) {
        if (res.length > 0) {
            var opt = "";//"<option value='-1'>请选择</option>";
            for (var i = 0; i < res.length; i++) {
                opt += '<option value=' + res[i].AliasSubkeyBase + '>' + res[i].CampAttrName + '</option>';
            }
            //return opt;
